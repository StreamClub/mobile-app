...
export const ArtistFilmografy = (params: ArtistFilmografyParams) => {
    ...
    return (
        <View style={styles.container}>
            <Pressable style={[styles.sectionTitle, {width: screenWidth}]} onPress={() => setShowCastSection(!showCastSection)}>
                <Icon source={castTitleSymbol} size={30}/>
                <TitleText body={castSectionTitle} size='medium' />
            </Pressable>
            {showCastSection && castTuples.map((tuple, index) => renderRow(tuple, index))}
            
            <Pressable style={[styles.sectionTitle, {width: screenWidth}]} onPress={() => setShowCrewSection(!showCrewSection)}>
                <Icon source={crewTitleSymbol} size={30}/>
                <TitleText body={crewSectionTitle} size='medium' />
            </Pressable>
            {showCrewSection && crewTuples.map((tuple, index) => renderRow(tuple, index))}
        </View>
    )
}
