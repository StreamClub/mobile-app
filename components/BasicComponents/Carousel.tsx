...
import { TmdbImage, TmdbImageParams, TmdbImageType } from './TmdbImage';
...
const isCloseToRight = ({ layoutMeasurement, contentOffset, contentSize }: NativeScrollEvent) => {
    return layoutMeasurement.width + contentOffset.x >= contentSize.width - EDGE_PROXIMITY_THRESHOLD;
};

const isCloseToLeft = ({ layoutMeasurement, contentOffset, contentSize }: NativeScrollEvent) => {
...
export const Carousel = (params: CarouselParams) => {
    ...
    const sideContainerHeight = params.itemStyle.height + (params.itemStyle.margin * 2)
    ...
    const onLayout = (event: LayoutChangeEvent) => {
        ...
        if (containerWidth + visibleWidth < visibleWidth * params.items.length) {
            setShowRightIcon(true)
        }
      };
    ...
    return (
        <View style={[styles.container, params.containerStyle]}>
            <Animated.View style={[styles.sideIconContainerLeft, {opacity: opacityLeft, height: sideContainerHeight}]}>
                <Image source={leftIcon} style={[styles.sideIconLeft, {top: sideIconTop}]} />
            </Animated.View>
            ...
            <Animated.View style={[styles.sideIconContainerRight, {opacity: opacityRight, height: sideContainerHeight}]}>
                <Image source={rightIcon} style={[styles.sideIconRight, {top: sideIconTop}]} />
            </Animated.View>
        </View>
    )
}
